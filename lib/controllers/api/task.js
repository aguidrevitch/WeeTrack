(function () {
    ...
    module.exports = {
        setup: function (app, authorizedOnly) {
            ...
            /* watch */
            app.post('/api/task/:workspace/:id/watch', function (req, res) {
                var condition = { id: req.params.id };
                ...
                Task.findOneWithAccess(req.user, ['admin', 'admincc', 'cc', 'owner'], condition, function (err, workspace) {
                    if (err) {
                        res.error(err);
                    } else if (!workspace) {
                        res.error(new Error('Access denied'));
                    } else {
                        task.enable(req.user, 'watch', function (err, model) {
                            if (err) {
                                res.modal(err);
                            } else {
                                res.json(model);
            ...
            /* unwatch */
            app.post('/api/task/:workspace/:id/unwatch', function (req, res) {
                var condition = { id: req.params.id };
                ...
                Task.findOneWithAccess(req.user, ['admin', 'admincc', 'cc', 'owner'], condition, function (err, workspace) {
                    if (err) {
                        ...
                        task.disable(req.user, 'watch', function (err, model) {
                            if (err) {
                                res.modal(err);
                            } else {
                                res.json(model);
