                ...
                if (req.query.owner) {
                    ...
                    mongoose.model('User').findByIdOrEmail(req.query.owner, function (err, user) {
                        if (err) {
                            ...
                            mongoose.model('Task').findWithAccess(user, ['owner'], condition, function (err, models) {
                                if (err) {
                                    res.error(err);
                                } else {
                                    res.json(models);
                    ...
                    mongoose.model('Task').findWithAccess(req.user, perms, condition, function (err, models) {
                        if (err) {
                            res.error(err);
                        } else {
                            res.json(models);
