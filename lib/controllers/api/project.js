(function () {
    ...
    module.exports = {
        setup: function (app) {
            ...
            app.post('/api/project/:id/watch', function (req, res) {
                Project.findByIdWithAccess(req.user, ['admin', 'admincc', 'cc', 'owner'], req.params.id, function (err, project) {
                    if (err) {
                        ...
                        project.enable(req.user, 'watch', function (err, model) {
                            if (err) {
                                res.modal(err)
                            } else {
                                res.json(model)
            ...
            app.post('/api/project/:id/unwatch', function (req, res) {
                Project.findByIdWithAccess(req.user, ['admin', 'admincc', 'cc', 'owner'], req.params.id, function (err, project) {
                    if (err) {
                        res.modal(err);
                    } else if (!project) {
                        ...
                        project.disable(req.user, 'watch', function (err, model) {
                            if (err) {
                                res.modal(err)
                            } else {
                                res.json(model)
