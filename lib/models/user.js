(function () {
    ...
    UserSchema.statics.authorize = function (email, password, callback) {
        this.model('User').findOne({
            email: email
        }, {_id: 1, name: 1, email: 1, password: 1}, function (err, user) {
            if (err) {
                callback(err);
                return;
            }
    ...
    UserSchema.methods.refresh = function (callback) {
        var self = this;
        this.model('User').findById(this._id, function (err, user) {
            if (err) {
                callback(err)
            } else if (!user) {
                callback(new Error("User not found"))
            } else {
                self.set(user.toObject());
                callback(null, user);
            }
        });
    };
