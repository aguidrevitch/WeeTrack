...
module.exports = function (schema, options) {
    ...
    schema.methods.flattenPermissions = function () {
        var perms = {};
        _.each(options.permissions, function (multiple, perm) {
            if (multiple) {
                perms[perm] = [];
    ...
    schema.methods.grant = function (callback) {
        ...
        this.getAllowedSubjects(false, _.bind(function (err, allowed) {
            ...
            _.extend(this, this.flattenPermissions());
            ...
            this._grants = [];
            this._revokes = [];
            var grant = _.bind(function (multiple, perm, user_id) {
                if (user_id) {
                    grants.push(_.bind(function (callback) {
                        // console.log("granting " + perm + " to " + user_id);
                        Subject.grant(user_id, this.path, [perm], _.bind(function (err, user) {
                            if (err) {
                                callback(err)
                            } else {
                                this._grants.push({ perm: perm, user: user});
                                callback(null, user);
                            }
                        }, this));
                    }, this));
            ...
            var revoke = _.bind(function (multiple, perm, user_id) {
                // cannot revoke own permissions
                if (user_id && (!multiple || this.updater._id != user_id))
                    revokes.push(_.bind(function (callback) {
                        // console.log("revoking " + perm + " from " + user_id);
                        Subject.revoke(user_id, this.path, [perm], _.bind(function (err, user) {
                            if (err) {
                                callback(err)
                            } else {
                                this._revokes.push({ perm: perm, user: user});
                                callback(null, user);
                            }
                        }, this));
                    }, this));
            ...
            _.each(options.permissions, function (multiple, perm) {
                if (multiple) {
                    _.each(_.difference(this[perm], allowed[perm]), _.bind(grant, this, true, perm));
                }
            }, this);

            _.each(options.permissions, function (multiple, perm) {
                if (multiple)
                    _.each(_.difference(allowed[perm], this[perm]), _.bind(revoke, this, true, perm));
            }, this);

            _.each(options.permissions, function (multiple, perm) {
                if (!multiple && this[perm] !== allowed[perm]) {
                    if (allowed[perm]) revoke(false, perm, allowed[perm]);
                    if (this[perm]) grant(false, perm, this[perm]);
                }
            }, this);
        ...
        this.findOne.apply(this, [criteria, fields, options, function (err, rec) {
            if (err) {
                ...
                if (rec) {
                    rec.updater = user;
                    rec.permissions = user.getAccessTo(rec.path);
                    rec.getAllowedSubjects(true, function (err, allowed) {
                        if (err) {
                            callback(err);
                        } else {
                            _.extend(rec, allowed);
                            callback(null, rec);
                        }
                    });
